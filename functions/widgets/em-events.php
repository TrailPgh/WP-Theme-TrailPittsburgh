...
class themnific_EM_Widget extends WP_Widget {
    ...
    function __construct() {
    	$this->defaults = array(
    		'title' => esc_html__('Events','dotorg'),
    		'scope' => 'future',
    		'order' => 'ASC',
    		'limit' => 5,
    		'category' => 0,
    		'format' => '<li class="p-border">#_EVENTIMAGE{60,60}#_EVENTLINK<ul class="meta"><li>#_EVENTDATES</li><li>#_LOCATIONTOWN</li></ul></li>',
    		'nolistwrap' => false,
    		'orderby' => 'event_start_date,event_start_time,event_name',
			'all_events' => 0,
			'all_events_text' => esc_html__('all events','dotorg'),
			'no_events_text' => '<li>'.esc_html__('No events','dotorg').'</li>'
    	);
		$this->em_orderby_options = apply_filters('em_settings_events_default_orderby_ddm', array(
			'event_start_date,event_start_time,event_name' => __('start date, start time, event name','dotorg'),
			'event_name,event_start_date,event_start_time' => __('name, start date, start time','dotorg'),
			'event_name,event_end_date,event_end_time' => __('name, end date, end time','dotorg'),
			'event_end_date,event_end_time,event_name' => __('end date, end time, event name','dotorg'),
		)); 
    	$widget_ops = array('description' => esc_html__( "Customized Widget - Display a list of events on Events Manager.", 'dotorg') );
        parent::__construct(false, $name = esc_html__( 'Themnific - Events', 'dotorg'), $widget_ops);	
    }
    ...
    function widget($args, $instance) {
		...
		//output events
		echo '<ul class="tmnf_events_widget">';
		if ( count($events) > 0 ){
		...
		if ( !empty($instance['all_events']) ){
			$events_link = (!empty($instance['all_events_text'])) ? em_get_link($instance['all_events_text']) : em_get_link(__('all events','dotorg'));
    ...
    function form($instance) {
        ...
        ?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title', 'dotorg'); ?>: </label>
			<input type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo esc_attr($instance['title']); ?>" class="widefat" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('limit'); ?>"><?php esc_html_e('Number of events','dotorg'); ?>: </label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('scope'); ?>"><?php esc_html_e('Scope','dotorg'); ?>: </label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id('order'); ?>"><?php esc_html_e('Order By','dotorg'); ?>: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('order'); ?>"><?php esc_html_e('Order','dotorg'); ?>: </label>
			<select id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>" class="widefat">
				<?php 
				$order_options = apply_filters('em_widget_order_ddm', array(
					'ASC' => __('Ascending','dotorg'),
					'DESC' => __('Descending','dotorg')
				)); 
				?>
		...
		<p>
            <label for="<?php echo $this->get_field_id('category'); ?>"><?php esc_html_e('Category IDs','dotorg'); ?>: </label>
            ...
            <em><?php esc_html_e('1,2,3 or 2 (0 = all)','dotorg'); ?> </em>
        </p>
        <p>
			<label for="<?php echo $this->get_field_id('all_events'); ?>"><?php esc_html_e('Show all events link at bottom?','dotorg'); ?>: </label>
		...
		<p id="<?php echo $this->get_field_id('all_events'); ?>-section">
			<label for="<?php echo $this->get_field_id('all_events'); ?>"><?php esc_html_e('All events link text?','dotorg'); ?>: </label>
		...
		<em><?php echo sprintf( esc_html__('The list is wrapped in a %s tag, so if an %s tag is not wrapping the formats below it will be added automatically.','dotorg'), '<code>&lt;ul&gt;</code>', '<code>&lt;li&gt;</code>'); ?></em>
        <p>
			<label for="<?php echo $this->get_field_id('format'); ?>"><?php esc_html_e('List item format','dotorg'); ?>: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('no_events_text'); ?>"><?php _e('No events message','dotorg'); ?>: </label>
...
add_action('widgets_init', create_function('', 'return register_widget("themnific_EM_Widget");'));
